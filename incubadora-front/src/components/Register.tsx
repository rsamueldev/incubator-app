...
export const Register = ({ onBack, onRegisterSuccess }: any) => {
  ...
  const handleRegister = (e: React.FormEvent) => {
    e.preventDefault();
    // Guardamos en la memoria del navegador para que el Login lo reconozca
    localStorage.setItem('userEmail', email);
    localStorage.setItem('userPass', pass);
    
    alert('Cuenta creada con éxito. Ahora puedes iniciar sesión con estos datos.');
    onRegisterSuccess();
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#09090b] text-white p-6">
      <div className="w-full max-w-[400px] space-y-8 bg-[#18181b] p-8 rounded-[2.5rem] border border-[#27272a] shadow-2xl">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button className="w-full bg-emerald-500 text-black font-black p-4 rounded-2xl hover:bg-emerald-400 active:scale-95 transition-all">
            Crear Cuenta
          </button>
        </form>
      </div>
